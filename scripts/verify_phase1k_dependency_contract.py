...
def main() -> int:
    ...
    # Slimefun has no hard plugin dependency and does not impersonate external plugins.
    ...
    # Optional plugin APIs must remain compileOnly and transitive-free.
    ...
    # Authlib is compile-only platform glue and must not be bundled either.
    require(
    ...
    # Libraries intentionally bundled by Slimefun must stay relocated into Slimefun's private namespace.
    for token, label in PRIVATE_RELOCATIONS:
        require(token in build_gradle, f"Private relocation missing for {label}", failures)

    # GuizhanLib is an internal relocated library only; it is not a replacement for GuizhanLibPlugin.
