...
def main() -> int:
    ...
    try:
        ...
        phase_match = re.fullmatch(r"Core Platform Phase 1([A-Z])", phase)
        req(
            bool(phase_match and phase_match.group(1) >= "H"),
            "Support contract phase must be Phase 1H or later",
            failures,
        )
        ...
        for rel, expected in guarded.items():
            ...
            if path.is_file() and current == "4.1.26":
    ...
    report.write_text(
        ...
        "- historical 4.1.26 Phase 1H normal-core hash invariant is retained; later releases are validated by later phase verifiers\n",
        encoding="utf-8",
    )
    print(report.read_text(encoding="utf-8"), end="")
    return 0
