...
def main() -> int:
    ...
    require(bridge_sync, "cache(location, powered, gameTime, cached)", "successful validation refresh with cached entry reuse")
    ...
    require(bridge_cache, "phase - Math.floorMod(nextValidationTick, REVALIDATE_INTERVAL_TICKS)", "phase-aligned validation offset")
    require(bridge_cache, "REVALIDATE_INTERVAL_TICKS);", "validation offset modulus")
    require(bridge_cache, "if (cached != null)", "existing power-state cache entry reuse")
    require(bridge_cache, "cached.powered = powered", "in-place cached desired-state update")
    require(bridge_cache, "cached.nextValidationTick = nextValidationTick", "in-place validation deadline update")
    ...
    require(bridge_cache, "LAST_APPLIED_STATE.putIfAbsent(location.clone(), created)", "stable key cloned only on first insertion")
    require(bridge_compact, "private volatile boolean powered", "thread-visible cached power state")
    require(bridge_compact, "private volatile long nextValidationTick", "thread-visible cached validation deadline")
