...
PLUGIN_JAR="${1:?Usage: legacy_1_21_11_runtime_smoke.sh <slimefun-jar> [work-directory]}"
WORK_DIR="${2:-build/legacy-1.21.11-runtime-smoke}"
...
if [[ "$MC_VERSION" != "1.21.11" ]]; then
    echo "This compatibility harness is intentionally pinned to Minecraft 1.21.11; got ${MC_VERSION}." >&2
    exit 1
fi
...
enable-query=false
enable-rcon=false
motd=Slimefun Legacy 1.21.11 compatibility smoke
PROPERTIES
...
Slimefun Legacy ${SOFTWARE_NAME} 1.21.11 runtime smoke: PASS
