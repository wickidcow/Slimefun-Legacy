...
abstract class AbstractEnchantmentMachine extends AContainer {
    ...
    protected boolean canProgressOperation(BlockMenu menu, CraftingOperation operation) {
        boolean valid = EnchantmentMachineRuntime.inputsMatchSnapshots(menu, getInputSlots(), operation.getIngredients());
        if (!valid) {
            EnchantmentMachineRuntime.status(
                    menu,
                    Material.BARRIER,
                    "&eProcessing paused",
                    "&7Restore the original item and book",
                    "&7to continue without consuming energy.");
        }
        return valid;
    }
