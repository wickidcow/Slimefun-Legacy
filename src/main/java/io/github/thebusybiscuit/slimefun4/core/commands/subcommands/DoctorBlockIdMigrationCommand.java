...
final class DoctorBlockIdMigrationCommand {
    ...
    private void scan(CommandSender sender) {
        ...
        if (result.busy()) {
            send(sender, "&eBlock storage is currently busy. No block-ID plan was created; try again after pending writes drain.");
            return;
        }

        PersistedBlockIdMigrationPlan plan = result.plan();
        send(sender, "&6Slimefun Doctor Persisted Block-ID Plan");
        send(sender, "&7Persisted normal-block records scanned: &e" + plan.getScannedRecords());
        ...
        send(sender, "&eLoaded candidates are skipped during execution; unload those chunks and re-scan to migrate them later.");
    ...
    private void executePlan(CommandSender sender, String[] args) {
        ...
        if (summary.loaded() > 0) {
            send(sender, "&eLoaded records were intentionally untouched. Unload those chunks, then run a fresh scan.");
        }
    ...
    private void sendUsage(CommandSender sender) {
        ...
        send(sender, "&7Scan is storage-level and read-only; it never loads chunks. Execute requires its fresh fingerprint.");
    }
