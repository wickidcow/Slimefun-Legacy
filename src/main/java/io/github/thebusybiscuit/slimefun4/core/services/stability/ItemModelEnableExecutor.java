...
/**
 * Safely adopts Slimefun Legacy's bundled resource-pack model map on existing ItemStacks.
 *
 * <p>The executor adds the bundled value only when the first model-float lane is empty. A different
 * existing first float is treated as a conflict and left untouched. Other modern CustomModelData
 * lanes are preserved. This traversal is explicit-only and is never called by automatic Doctor repair.</p>
 */
final class ItemModelEnableExecutor extends ItemDoctorTraversalExecutor {
    ...
    ItemModelEnableExecutor(boolean repair) {
        this.repair = repair;
    ...
    private boolean inspectItem(@Nullable ItemStack item, @Nonnull ItemDoctorReport report, int depth) {
        ...
        try {
            ...
            report.failure();
            Slimefun.logger().log(Level.WARNING, "Hosted-pack Doctor skipped a failing ItemStack.", exception);
            return changed;
        }
    }
    ...
    private boolean inspectCandidate(ItemStack item, String slimefunId, ItemDoctorReport report) {
        ...
        int configuredModel = Slimefun.getItemTextureService().getModelData(slimefunId);
        if (configuredModel != 0 && configuredModel != bundledModel) {
            return false;
        }
        ...
        if (!floats.isEmpty()) {
            if (Float.compare(floats.get(0), (float) bundledModel) != 0) {
        ...
        try {
            ...
            if (resultingId.isEmpty() || !slimefunId.equals(resultingId.get())) {
                ...
                Slimefun.logger().warning(
                        "Hosted-pack Doctor refused a repair because the Slimefun item ID changed unexpectedly.");
                return false;
            }
            ...
            Slimefun.logger().log(
                    Level.WARNING,
                    "Hosted-pack Doctor could not safely add the bundled model value to " + slimefunId + '.',
                    exception);
            return false;
        }
    }
    ...
    private static Map<String, Integer> loadBundledModels() {
        ...
        if (stream == null) {
            Slimefun.logger().warning("Hosted-pack Doctor could not find the bundled item-models.yml resource.");
            return models;
        }
        ...
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(stream, StandardCharsets.UTF_8))) {
            ...
            Slimefun.logger().log(Level.SEVERE, "Hosted-pack Doctor could not load bundled item-model mappings.", exception);
        }
