...
abstract class BaseFastMachine(
...
) : MenuBlock(itemGroup, itemStack, recipeType, recipe), EnergyNetComponent, MachineRecipeDisplayItem {
    ...
    /**
     * Exposes FastMachines recipes directly to Slimefun Legacy's Enhanced Guide.
     *
     * This uses the public machine-recipe API instead of cross-plugin reflection, ensuring the
     * Machine Recipes button is available whenever this machine has enabled recipes in the
     * player's current world.
     */
    override fun getMachineRecipeDisplays(world: World): List<MachineRecipeDisplay> =
        recipes.mapNotNull { recipe ->
            if (recipe.isDisabledIn(world)) {
                return@mapNotNull null
            }

            val outputs = recipe.outputs.filter { output ->
                output.type != Material.AIR && output.amount > 0
            }
            if (outputs.isEmpty()) {
                return@mapNotNull null
            }

            val builder = MachineRecipeDisplay.builder()
                .layout(MachineRecipeLayout.SHAPELESS)
                .label("FastMachines recipe")

            for (input in recipe.inputs) {
                val choices = input.choices.mapNotNull { (wrapper, requiredAmount) ->
                    if (requiredAmount <= 0 || wrapper.baseItem.type == Material.AIR) {
                        null
                    } else {
                        wrapper.baseItem.clone().apply { amount = requiredAmount }
                    }
                }
                ...
                builder.addIngredient(MachineRecipeIngredient(choices))
            }
