...
Current release: **4.1.52 — Storage safety, addon integration & 26.3 compatibility**. ·
...
Slimefun Legacy 4.1.52 uses the exact released 4.1.51 tag commit as its release-blocking previous-stable compatibility baseline. Required addons that work against 4.1.51 but regress against the 4.1.52 candidate block release. The historical 4.1.15 floor remains advisory for long-term drift visibility. CI-only addon repository coverage may be broader than `/sf versions`; runtime recognition stays curated and never treats CI monitoring as proof for an exact installed addon build.
