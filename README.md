...
Slimefun Legacy 4.1.55 uses the latest released previous-stable line as its release-blocking compatibility baseline. Required addons that work against the previous stable release but regress against the refreshed 4.1.55 build block release. The historical 4.1.15 floor remains advisory for long-term drift visibility. CI-only addon repository coverage may be broader than `/sf versions`; runtime recognition stays curated and never treats CI monitoring as proof for an exact installed addon build.
